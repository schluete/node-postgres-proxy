...
ProxyServer.prototype.handleRequest = function (req, resp) {
  ...
  req.on('end', function () {
    self.databaseConnection(req, function(err, client) {
      if(err) {
        // simple way to destinguish between database and path errors
        if(err.split(' ') == '404')
            sendError(resp, err, 404);
        else       
            sendError(resp, err, 502);
      }
      else {
        client.query(query, function (err, rs) {
          if(err) {
            client.query('rollback');
            sendError(resp, err.message, 400);
          }
          else {
            client.query('commit');
            rs.success = true;
...
ProxyServer.prototype.databaseConnection = function (req, callback) {
  ...
  if (path.split('/')[1] != 'sql')
      return callback('404 - unknown path');
  var dbName = path.split('/')[2];
  if(!(dbName in this.config['databases']))
    return callback('404 - configuration for database "' + dbName + '" does not exist');
